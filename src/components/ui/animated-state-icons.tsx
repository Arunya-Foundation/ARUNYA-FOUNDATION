...
/* ─── 13. HOME ─── house with chimney smoke */
export function HomeIcon({ size = 40, color = "currentColor", className, duration = 2200 }: StateIconProps) {
    const active = useAutoToggle(duration);
    return (
        <svg viewBox="0 0 40 40" fill="none" className={cn("", className)} style={{ width: size, height: size }}>
            <path d="M6 18L20 6l14 12" stroke={color} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" />
            <path d="M10 14v18a2 2 0 002 2h16a2 2 0 002-2V14" stroke={color} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" />
            <path d="M16 34V22a2 2 0 012-2h4a2 2 0 012 2v12" stroke={color} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" />
            <motion.path d="M26 6c0-2 2-2 2-4" stroke={color} strokeWidth={2} strokeLinecap="round"
                animate={active ? { pathLength: 1, opacity: 0, y: -4 } : { pathLength: 0, opacity: 1, y: 0 }}
...
/* ─── 14. INFO / ABOUT ─── info circle with bounce */
export function InfoIcon({ size = 40, color = "currentColor", className, duration = 2000 }: StateIconProps) {
    const active = useAutoToggle(duration);
    return (
        <svg viewBox="0 0 40 40" fill="none" className={cn("", className)} style={{ width: size, height: size }}>
            <circle cx="20" cy="20" r="14" stroke={color} strokeWidth={2} />
            <motion.path d="M20 18v8" stroke={color} strokeWidth={2} strokeLinecap="round"
                animate={active ? { y: 2, scaleY: 0.8 } : { y: 0, scaleY: 1 }}
                transition={{ duration: 0.3 }}
                style={{ transformOrigin: "20px 22px" }}
            />
            <motion.circle cx="20" cy="13" r="1.5" fill={color}
